...
class AdminYonetim extends Controller
{
    ...
    {
      $dinamikModul=Modeller::whereDurum(1)->whereSeflink($modul)->first();
      if($dinamikModul)
      {
        $dinamikModel="App\Models\\".ucfirst($dinamikModul->seflink);
        $veriler=$dinamikModel::get();
        return view("admin.include.liste",compact(['veriler','dinamikModul']));//,'dinamikModul'
    ...
    {
      $modulBilgisi=Modeller::whereDurum(1)->whereSeflink($modul)->first();
    ...
    public function eklePost($modul,Request $request)
    {
      ...
      if($modulBilgisi)
      {
       ...
       return redirect()->route('ekle',$modulBilgisi->seflink)->with('basarili','İşleminizi başaralı');
      }
else
{
...
public function duzenlePost($modul,$id,Request $request)
{
  ...
  if($modulBilgisi)
  {
   ...
   return redirect()->route('duzenle',[$modulBilgisi->seflink,$id])->with('basarili','İşleminizi başaralı');
  }
else
{
...
public function sil($modul,$id)
  ...
  if($modulBilgisi && $kategoriBilgisi)
  {
    ...
    if($veriler)
    {
      ...
      return redirect()->back()->with('basarili','İşleminizi başaralı');
    }
    else
    {
...
$logoDosyasi=$request->file('logo');
   if(isset($logoDosyasi))
   {
    ...
    {
     $favicon=uniqid().".".$faviconDosyasi->getClientOriginalExtension();
...
return redirect()->route('ayarlar')->with('basarili','İşleminizi başaralı');
}
